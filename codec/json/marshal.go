...
func (c Codec) marshalModel(model mapping.Model) (marshaler, error) {
	...
	for _, sField := range mStruct.StructFields() {
		...
		case mapping.KindPrimary, mapping.KindAttribute, mapping.KindForeignKey:
			...
			if isZero && fieldAnnotation.IsOmitEmpty {
				continue
			}
			...
			result = append(result, keyValue{Key: fieldAnnotation.Name, Value: fieldValue})
		case mapping.KindRelationshipSingle:
			...
			if relation == nil && fieldAnnotation.IsOmitEmpty {
				continue
			}
			...
			result = append(result, keyValue{Key: fieldAnnotation.Name, Value: relationNode})
		case mapping.KindRelationshipMultiple:
			...
			if len(models) == 0 && fieldAnnotation.IsOmitEmpty {
				continue
			}
			...
			result = append(result, keyValue{Key: fieldAnnotation.Name, Value: relationNodes})
		}
	}
	return result, nil
}
