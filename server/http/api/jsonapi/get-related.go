...
func (a *API) handleGetRelated(mStruct *mapping.ModelStruct, relationField *mapping.StructField) http.HandlerFunc {
	...
	return func(rw http.ResponseWriter, req *http.Request) {
		...
		result, err := a.getRelationHandleChain(params, s, relatedScope, relationField)
		if err != nil {
			a.marshalErrors(rw, 0, httputil.MapError(err)...)
			return
		}
...
func (a *API) getRelationHandleChain(params *server.Params, s, relatedScope *query.Scope, relationField *mapping.StructField) (*codec.Payload, error) {
	modelHandler, hasModelHandler := a.handlers[s.ModelStruct]
	if hasModelHandler {
		beforeHandler, ok := modelHandler.(server.BeforeGetRelationHandler)
		if ok {
			if err := beforeHandler.HandleBeforeGetRelation(params, s, relatedScope, relationField); err != nil {
				return nil, err
			}
		}
	}
	...
	result, err := handler.HandleGetRelation(params.Ctx, *params, s, relatedScope, relationField)
	if err != nil {
		return nil, err
	}
	if hasModelHandler {
		if afterHandler, ok := modelHandler.(server.AfterGetRelationHandler); ok {
			if err = afterHandler.HandleAfterGetRelation(params, result); err != nil {
				return nil, err
			}
		}
	}
