...
func fieldsColumn(field *mapping.StructField) (*Column, error) {
	...
	if !ok {
		...
		return nil, errors.NewDetf(controller.ClassInternal, "no column found in the field's '%s' store", field.Name())
	}
	...
	if !ok {
		...
		return nil, errors.NewDetf(controller.ClassInternal, "stored column for field: '%s' is not a *migrate.Column", field.Name())
	}
